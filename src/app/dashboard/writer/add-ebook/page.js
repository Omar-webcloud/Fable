...
export default function AddEbookPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.title || !form.description || !form.price || !form.genre) {
    ...
    try {
      ...
      if (imageFile) {
        try {
          ...
          toast.error("Image upload failed, continuing without cover");
        }
      }

      await ebookService.create({
        title: form.title,
      ...
      toast.error(err.response?.data?.error || "Failed to create ebook");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      <h1 className="mb-6 text-2xl font-bold text-dark">Add New Ebook</h1>
      ...
      <form onSubmit={handleSubmit} className="mx-auto max-w-2xl space-y-6">
        <div className="rounded-xl bg-white p-6 shadow-sm">
          ...
          <div className="mb-6">
            ...
            <div className="flex items-start gap-4">
              <div className="flex h-40 w-28 items-center justify-center overflow-hidden rounded-lg border-2 border-dashed border-gray-300 bg-gray-50">
                {imagePreview ? (
              ...
              <div className="flex-1">
                <input type="file" accept="image/*" onChange={handleImageChange} className="text-sm text-gray-500 file:mr-3 file:rounded-lg file:border-0 file:bg-primary/10 file:px-4 file:py-2 file:text-sm file:font-medium file:text-primary hover:file:bg-primary/20" />
                <p className="mt-2 text-xs text-gray-400">PNG, JPG up to 5MB. Uploaded to imgBB.</p>
              </div>
            </div>
          </div>
          ...
          <div className="mb-4">
            ...
            <textarea rows={10} value={form.fullContent} onChange={(e) => update("fullContent", e.target.value)}
              className="w-full rounded-lg border border-gray-300 px-4 py-2 font-mono text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20"
              placeholder="Write the full ebook content here..." />
          </div>
        </div>
        ...
        <div className="flex gap-3">
          <button type="submit" disabled={loading}
            ...
            {loading ? <LoadingSpinner size="sm" /> : "Create Ebook"}
          </button>
